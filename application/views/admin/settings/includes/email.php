	...
	<div role="tabpanel" class="tab-pane active" id="email_config">
		...
		<div class="form-group">
			...
			<div class="radio radio-inline radio-primary">
				...
				<label for="codeigniter">Code</label>
			</div>
			<hr />
			<?php if(get_option('email_protocol') == 'mail'){ ?>
				<div class="alert alert-warning">
					Giao thức "Email" không phải là giao thức được khuyến nghị để gửi email, bạn nên cân nhắc việc định cấu hình giao thức "SMTP" để tránh bất kỳ sự gián đoạn và sự cố gửi nào.
				</div>
			<?php } ?>
		...
		<div class="form-group">
			<div class="input-group">
				...
				<div class="input-group-btn">
					<button type="button" class="btn btn-default test_email p7">Kiểm tra</button>
				</div>
			</div>
		</div>
	...
	<div role="tabpanel" class="tab-pane" id="email_queue">
		<?php if(get_option('cron_has_run_from_cli') != '1') { ?>
			<div class="alert alert-danger">
				Tính năng này yêu cầu một công việc cron được định cấu hình đúng cách. Trước khi kích hoạt tính năng này, hãy đảm bảo rằng <a href="<?php echo admin_url('settings?group=cronjob'); ?>">Tác vụ tự động</a> được định cấu hình như phần giải thích trong tài liệu.
			</div>
		<?php } ?>
		<?php render_yes_no_option('email_queue_enabled','email_queue_enabled','Để tăng tốc độ xử lý email, hệ thống sẽ thêm email cần gửi vào hàng đợi và thực hiện gửi theo lịch tác vụ tự động. Hãy chắc chắn rằng tác vụ tự động đã được thiết lập và hoạt động.'); ?>
		<hr />
		<?php render_yes_no_option('email_queue_skip_with_attachments','email_queue_skip_attachments','Nhiều khả năng bạn sẽ gặp sự cố với hàng đợi email nếu hệ thống cần thêm các tệp lớn vào hàng đợi. Nếu bạn định sử dụng tùy chọn này, hãy tham khảo ý kiến của quản trị viên máy chủ/nhà cung cấp dịch vụ lưu trữ để tăng tùy chọn max_allowed_packet và wait_timeout trong cấu hình máy chủ của mình, nếu không khi tùy chọn này được đặt thành có, hệ thống sẽ không thêm email có tệp đính kèm vào hàng đợi và sẽ được gửi ngay lập tức.'); ?>
	</div>
</div>
