...
abstract class App_import
{
     ...
     * @var integer
     */
    private $warningOnTotalRows = 5000000;
    ...
    public function maxInputVarsWarningHtml()
    {
        ...
        if (($max_input > 0 && !is_null($totalRows) && $totalRows >= $max_input)) {
            ...
            <div class=\"alert alert-warning\">
                Nhà cung cấp dịch vụ lưu trữ của bạn đã đặt cấu hình PHP <b>max_input_vars</b> thiết lập để $max_input.<br/>
                Yêu cầu nhà cung cấp dịch vụ lưu trữ của bạn tăng cấu hình <b>max_input_vars</b> lên $totalRows hoặc cao hơn để nhập số lượng dữ liệu bạn đang cố nhập, nếu không, hãy thử chia nhỏ các hàng tệp .csv nhập và cố gắng nhập ít hàng hơn.
              </div>";
        }
    ...
    public function downloadSampleFormHtml()
    {
        ...
        $form .= '<button type="submit" class="btn btn-success">Tải file mẫu</button>';
        $form .= '<hr />';
        $form .= form_close();
    ...
    {
        return ' <h4 class="bold">Dữ liệu mô phỏng <small class="text-info">tối đa ' . $this->maxSimulationRows . ' hàng được hiển thị</small></h4>
              <p class="bold">Nếu bạn hài lòng với kết quả, hãy tải tệp lên lại và nhấp vào Nhập tập tin.</p>';
    }
    ...
    public function downloadSample()
    {
        ...
        header('Content-Disposition: attachment; filename="file_import_mau.csv";');
    ...
    protected function readFileRows()
    {
        ...
        if ($this->totalRows <= 1) {
            set_alert('warning', 'Không đủ hàng để nhập');
            redirect($this->failureRedirectURL());
        }
        ...
        if ($this->isSimulation() && $this->totalRows > $this->warningOnTotalRows) {
            $warningMsg = 'Khuyến nghị chia tệp CSV thành các tệp nhỏ hơn. Lời khuyên của chúng tôi là ' . $this->warningOnTotalRows . ' hàng, tệp CSV của bạn có ' . $this->totalRows;
    ...
    {
        $this->addImportGuidelinesInfo('Nếu cột <b>có chứa dữ liệu ngày tháng</b>, cần đảm bảo định dạng ngày tháng là <b>Y-m-d (' . date('Y-m-d') . ').</b>');

        $this->addImportGuidelinesInfo('Tệp dữ liệu CSV phải có <b>định dạng và tiêu đề cột như bảng bên dưới</b>. Bạn cũng cần đảm bảo rằng dữ liệu nhập tương thích bộ mã <b>UTF-8</b> để tránh các <b>lỗi phát sinh</b>.');
    }
