...
foreach ($rResult as $aRow) {
    ...
    $nameRow .= (staff_can('create', 'tasks') && $aRow['approved'] == 1 && $aRow['source'] != 'internal_staff_crm') ?
        ' | <a data-toggle="tooltip" title="' . _l('appointments_create_task_tooltip') . '" href="#" data-customer-id="' . appointly_get_contact_customer_id($aRow['contact_id']) . '" data-source="' . $aRow['source'] . '" data-contact-id="' . $aRow['contact_id'] . '" data-name="' . $aRow['name'] . '" onclick="new_task_from_relation_appointment(this); return false;">' . _l('appointments_create_task_tooltip') . '</a>'
        : '';
    ...
    if (staff_can('edit', 'appointments') || staff_can('create', 'appointments') || staff_appointments_responsible()) {
        ...
        if ($needs_approval) {
            $outputStatus .= '<li><a href="" onclick="markAppointmentAsApproved(' . $aRow['id'] . '); return false" href="">' . _l('task_mark_as', 'Đã chấp nhận') . '</a></li>';
        }

        if ($aRow['cancelled'] == 0 && $aRow['finished'] == 0) {
            if ($aRow['created_by'] == get_staff_user_id() || staff_appointments_responsible()) {
                $outputStatus .= '<li><a href="" onclick="markAppointmentAsCancelled(' . $aRow['id'] . '); return false" id-"cancelAppointment">' . _l('task_mark_as', 'Đã hủy') . '</a></li>';
            }
        }

        if ($aRow['finished'] == 0 && $aRow['cancelled'] == 0 && $aRow['approved'] != 0) {
            if ($aRow['created_by'] == get_staff_user_id() || staff_appointments_responsible()) {
                $outputStatus .= '<li><a href="" onclick="markAppointmentAsFinished(' . $aRow['id'] . '); return false" id="markAsFinished">' . _l('task_mark_as', 'Đã kết thúc') . '</a></li>';
            }
        }

        if ($aRow['cancelled'] == 1 && $aRow['finished'] == 0) {
            if ($aRow['created_by'] == get_staff_user_id() || staff_appointments_responsible()) {
                $outputStatus .= '<li><a href="" onclick="markAppointmentAsOngoing(' . $aRow['id'] . '); return false" id-"markAppointmentAsOngoing">' . _l('task_mark_as', 'Đang diễn ra') . '</a></li>';
            }
        }
        $outputStatus .= '</ul>';
        $outputStatus .= '</div>';
    ...
    if (!$_google_calendar_link && !$_outlook_calendar_link) {
        $options .= '<p class="text-muted">Chưa thêm</p>'; //lang
    }
