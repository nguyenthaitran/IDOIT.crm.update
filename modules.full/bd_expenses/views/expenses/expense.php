               ...
               <div class="panel-body">
                  <?php
                     if(isset($expense) && $expense->recurring_from != NULL){
                       ...
                       echo '<div class="alert alert-info">'._l('expense_recurring_from','<a href="'.admin_url('bd_expenses/list_expenses/'.$expense->recurring_from).'" target="_blank">'.$recurring_expense->category_name.(!empty($recurring_expense->expense_name) ? ' ('.$recurring_expense->expense_name.')' : '').'</a></div>');
                     }
                     ?>
                  ...
                  <div class="row">
                     ...
                     <div class="col-md-2 text-right">
                        <a href="<?php echo admin_url('bd_expenses/delete_expense_attachment/'.$expense->expenseid); ?>" class="text-danger _delete"><i class="fa fa fa-times"></i></a>
                     </div>
                     <?php } ?>
                  </div>
                  <?php } ?>
                  ...
                  <?php echo bd_expenses_render_custom_fields('expenses',$rel_id); ?>
        ...
        if (response.expenseid) {
         if(typeof(expenseDropzone) !== 'undefined'){
          if (expenseDropzone.getQueuedFiles().length > 0) {
            expenseDropzone.options.url = admin_url + 'bd_expenses/add_expense_attachment/' + response.expenseid;
